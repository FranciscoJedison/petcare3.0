...
const GerenciamentoUser = () => {
  ...
  const addUser = async () => {
    try {
      console.log('Enviando dados para criar usuário:', newUser); // Adicionado log para debug
  
      const response = await axios.post(`${API_URL}/usuario/inserir`, {
        nome: newUser.nome.trim(),
        email: newUser.email.trim(),
        senha: newUser.senha.trim(),
      ...
      console.error('Erro ao adicionar usuário:', error.response?.data?.error || error.message);
      // Você pode também exibir uma mensagem na interface para o usuário aqui, se quiser
    }
  };
    ...
    <PaperProvider>
      <SafeAreaView style={styles.container}>
        ...
        <Button
          ...
          buttonColor="#00635D" // Cor verde para o fundo do botão
          ...
          <ScrollView horizontal style={styles.scrollContainer}>
            ...
            <DataTable>
                    ...
                    {users.length > 0 ? (
                      ...
                      <DataTable.Row>
                        <DataTable.Cell><Text style={{color: 'white'}}>Nenhum usuário encontrado</Text></DataTable.Cell>
                      </DataTable.Row>
                    )}
                  </DataTable>
            </ScrollView>
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    padding: 16,
    backgroundColor: '#32312F', // Cor marrom claro
  },
  ...
  modalHeader: {
    backgroundColor: '#00635D', // Marrom claro
    alignItems: 'center',
    paddingVertical: 10,
